...
38. **The roster cache in `_get_adherence_agent_pks` is not invalidated on writes** — it only clears via its 300-second TTL (item 26). No `CACHES` backend is configured in `wfm/settings.py`, so Django falls back to the default per-process `LocMemCache`, which is not shared across worker processes.
...
56. **The Adherence supervisor dropdown does not filter in SQL.** `_get_adherence_agent_pks` accepts `supervisor_id` but uses it **only to build the cache key** — the query always scans the whole roster. Narrowing happens afterwards, in `_apply_supervisor_filter` and the `group=` param. Filtering the tab to one supervisor therefore makes its query no cheaper. Counterintuitive, and it cost real diagnostic time.
