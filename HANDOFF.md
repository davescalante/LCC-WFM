...
33. **`payroll_report` and `payroll_export` (`finance/views.py`) base their roster on `Q(track_attendance=True) | Q(five9_profiles__billable=True)`.** A separated agent with no billable Five9 profile fails this filter outright, regardless of the pay-window `.exclude()` layered on top — they drop out of their own final pay week despite satisfying the pay window.
