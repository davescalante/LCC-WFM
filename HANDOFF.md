...
| Tests | `scheduling/tests.py`, `erlang/tests.py`, `adherence/tests.py`, `finance/tests.py`, `nomina/tests.py` — run `python3 manage.py test`. As of the last commit: 466/466 passing |
...
57. **`OvertimeShift` has no uniqueness guard — known and unfixed.** Model `Meta` carries indexes only; the original `unique_together = ('agent','date')` was dropped in migration `0018` to allow intentional split OT. `overtime_week` creates rows with a bare `.create()` in a loop whose count comes straight from the POST body **with no cap**, unlike `open_ot_create`, which clamps to 20 — so a double-submit or an oversized value duplicates rows freely. Duplicated OT means duplicated hours, and those feed billing and payroll. Production currently has agents with **up to 15 OT rows on a single day**.
