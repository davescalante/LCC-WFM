...
Multiple Five9 logins per agent: `five9_username`/`five9_password`, `label`, `role_type`, `billable` (drives billing/payroll via `wfm.utils.get_billable_username_map`), `is_primary` (drives adherence DISPLAY hours via `wfm.utils.get_adherence_primary_resolver`, no fallback — see §10's most recent entry and CLAUDE.md's Adherence primary-account section). The two flags are read by two completely separate resolvers and must never be mixed. Replaces the legacy `Agent.five9_username` field.
...
rows on the agent's `is_primary` Five9 account** (`wfm.utils.get_adherence_primary_resolver`) —
no fallback to any other account, even when the primary has no row that day; an agent with no
account marked primary shows zero login. This is a completely separate flag/resolver from the
`billable` one the money engine above uses, and the two must never be mixed or given a fallback
into each other — see CLAUDE.md's Adherence primary-account section for the incident this
closed. The math itself is unchanged and now lives in one place, `adherence.views._compute_display_hours`
...
## 10. Recent Work (most recent first, as of `2b31326`)
