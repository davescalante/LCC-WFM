...
*Generated 2026-08-03 by reading the live codebase at `/Users/denisetijerina/Documents/LCC-WFM` (branch `main`, HEAD `87fc00c`). This is a structural map, not a code dump — verify exact line numbers against current code before quoting them, since the app changes fast (several commits landed the same day this was written).*
...
Multiple Five9 logins per agent: `five9_username`/`five9_password`, `label`, `role_type`, `billable` (drives billing/payroll via `wfm.utils.get_billable_username_map`) and `is_primary` (which account is primary **today**). The two are read by two completely separate resolvers and must never be mixed. Since `29203ec`, `is_primary` is written only by `five9_primary.sync_is_primary_from_history` and always equals today's winner from `Five9PrimaryPeriod` below — never assign it by hand. Adherence DISPLAY hours resolve the primary account **per day** from that history via `wfm.utils.get_adherence_primary_resolver`, with no fallback; see §10's `29203ec` entry and CLAUDE.md's Adherence primary-account section. Replaces the legacy `Agent.five9_username` field.

### `Five9PrimaryPeriod` (scheduling/models.py)
Append-only history of which Five9 account was an agent's primary over which days. `start_date` NULL means from the beginning, `end_date` NULL means open-ended, and **for any day the newest row covering it wins — newest being the highest `id`**. Kinds: `initial` (from the beginning: the seed, a new agent, the backfill), `switch` (from a date, open-ended), `always` (from the beginning and open-ended, superseding everything older), `past_period` (a closed range ending before today). Rows are never edited or deleted — a correction is a newer row. `profile` is `SET_NULL` and `five9_username` is a snapshot, so a rename follows the entry while a deleted account keeps counting the days it was primary. Written only through `scheduling.five9_primary.record_primary_period`.
...
```
**As of `d0b6402`, `login_seconds`/`not_ready_seconds` here are summed only from Daily Hours
rows on the agent's primary Five9 account** (`wfm.utils.get_adherence_primary_resolver`) —
no fallback to any other account, even when the primary has no row that day; an agent with no
primary shows zero login. **As of `29203ec` that resolution is per day**, from the
`Five9PrimaryPeriod` history (§3), so each day counts whichever account was primary *on that
date* — the resolver's signature and all of its callers are unchanged, and it stays bulk at at
most two queries. This is a completely separate selector from the `billable` flag the money
engine above uses, and the two must never be mixed or given a fallback into each other — see
CLAUDE.md's Adherence primary-account section for the incident this closed. The math itself is unchanged and now lives in one place, `adherence.views._compute_display_hours`
(pure arithmetic, no queries), called by the upload, re-match, and coding-recompute paths below,
plus the `recalculate_display_hours` management command that corrects previously-stored values
(§10's most recent entry).

As of `b2a4c8b` ("Fix stale actual_hours surviving Daily Hours delete/replace"), there's a `_reconcile_stale_actual_hours(upload_date)` helper wired into upload/re-match/delete of Daily Hours, all wrapped in `transaction.atomic()`, so deleting or replacing an upload no longer leaves stale `actual_hours` behind for agents who dropped off the new file — it zeroes anyone with no primary-account row that day and returns the counted-agent-id set, which `_zero_missing_scheduled` now takes directly rather than recomputing it a second time. This is a **display-layer fix only** — billing/payroll never read `actual_hours`; they always recompute from raw `DailyAgentHours` seconds independently (see §5, Known Quirk #2 conceptually).
...
## 10. Recent Work (most recent first, as of `29203ec`)
...
1. **Primary Five9 account history: date-aware adherence, super-admin locks on saved accounts, and past-date tools** (`29203ec`, migration `0055`) — finishes what `d0b6402` left a seam for. New **`Five9PrimaryPeriod`** is an append-only per-agent record of which account was primary over which days (`start_date` NULL = from the beginning, `end_date` NULL = open-ended); **for any day the newest row covering it wins, newest meaning highest `id`**. Kinds are `initial`, `switch`, `always` (supersedes everything older — for a setup mistake) and `past_period` (a closed range ending before today, after which older rows apply again). **Rows are never edited or deleted; a correction is a newer row**, which is what makes the Activity Log a complete account. The username is read live through the `profile` FK and snapshotted on the row, so a rename follows the entry while a deleted account (`SET_NULL`) keeps counting the days it was primary.

    `get_adherence_primary_resolver` is now date-aware **with no caller changed** — the `on_date` argument all eight call sites already passed was the seam, now honoured rather than ignored — and stays bulk at at most two queries regardless of agents or days, memoised per agent-day. `is_primary` still exists and is still read by exports, sorts and Billing v2 display, but nothing writes it directly any more: `five9_primary.sync_is_primary_from_history` sets it and it always equals today's winner, so every existing reader keeps working untouched. `scheduling/five9_primary.py` holds the unit; **`record_primary_period` is the single write path**, creating the row, syncing `is_primary`, recalculating exactly the days it covers and writing one `log_action` entry. **`ensure_seeded` runs first inside it and is load-bearing** — without the backfill, an entry that doesn't cover today (a past period) would be an agent's only history, today's winner would resolve to nobody and the sync would clear `is_primary`, silently zeroing that agent's adherence login; caught by a test during the build.

    Migration `0055` seeded one from-the-beginning entry per existing primary, **with no recalculation and no log entries**, since it reproduces the old answers exactly for every agent and date (pinned). An agent with 2+ accounts marked primary would collapse to the lowest `id` — `five9_account_setup_report` found **0** such agents. Recalculation reuses the command rather than copying it: `recalculate_display_hours._plan` became module-level `plan_display_hours`/`apply_display_hours` (behavior unchanged, its 13-test suite still passing), and `uncoded_extra_account_review` now resolves "primary" per day.

    Switching the primary records a starts-today entry and recalculates only from then on; **removing the account that is primary today is refused unless a replacement is named**; auto-primary writes from-the-beginning only when the agent has no history, else starts today. **Permissions, all server-side:** anyone who can edit users may add accounts and switch the primary starting today; super admins only may delete, rename or change Billable on a **saved** account, back-date a switch, use "always the primary", or use "Add a past primary period" — a JSON endpoint with four gates (login + portal middleware, super admin, ordinary CSRF, and an account queryset scoped to that agent so another agent's account writes nothing). A supervisor now sees a saved account's username as plain text, Billable as ✓/—, no Remove button, and one line reading "New primary counts starting today"; a super admin additionally gets a **Primary starting** date (default and `max` today) with an "always the primary" checkbox that greys it out, and the past-period form. The user detail page gained one muted timeline line, shown only when more than one account has actually been primary. **"Today" is the app's New York date**, so an evening switch after 9 PM Hermosillo (10 PM in winter) starts the next day — deliberate, not a second clock. Billing, payroll, Nómina and OT verification are untouched and `_get_billable_weekly_data` is pinned byte-for-byte identical after every kind of entry. Tests: 801 → 885.

2. **Fix the Activity Log and Agent History pages crashing on any entry with no recorded user** (`2b31326`) — the very next commit surfaced a bug the primary-account work below had introduced. `recalculate_display_hours`'s one summary `log_action(None, ...)` call was the first place in the app ever to pass `user=None` (every other of the ~70 call sites passes `request.user`), and it broke `/activity/` in production the same day it ran — 500 both unfiltered and filtered to that date, fine when filtered to any specific user (that filter excludes the null-user row entirely from the query). Reproduced directly against the real view and template before touching anything: `templates/scheduling/activity_log.html`'s `{{ entry.user.get_full_name|default:entry.user.username|default:"—" }}` crashes with `django.template.base.VariableDoesNotExist` when `entry.user` is `None`, because Django only silences a broken variable chain for the **primary** value piped into a filter — not for a filter's **argument**, and `entry.user.username` here is the argument to the first `default:`. A same-looking `{% if x %}...{% else %}...{% endif %}` guard two lines below it, for a different field, was already safe by construction — the two idioms look interchangeable and are not. Fixed on both pages with that guard instead: Activity Log's User column shows **System** for a null-user row; Agent History's Processed By (`AgentSeparation.processed_by`, nullable, currently always set in practice, found while checking this page for the same pattern rather than because it had actually fired) shows **—**. Existing entries render identically to before on both pages; no Python changed anywhere — `log_action`, `recalculate_display_hours` and `AuditLog` are untouched, since the nullable `user` field is correct as designed and the display side is what had to handle it. Both regression tests were confirmed erroring with the exact same `VariableDoesNotExist` against the pre-fix templates before either line was touched. Tests: 799 → 801.

3. **Adherence DISPLAY hours now count only an agent's primary Five9 account, with no fallback; a one-time command corrects stored history (`d0b6402`)** — closes a real production incident. Some agents run a second Five9 account (e.g. so their regular call-taking reports separately); time on it is never paid unless a supervisor codes it. The adherence pipeline (`upload_daily_file`, `rematch_daily_upload`, `_refresh_actual_hours`) used to select Five9 rows by the `billable` flag and, when the billable/primary account had no `DailyAgentHours` row for a day, **silently fell back to any other account** — so a day worked entirely on a non-primary account looked like a normal worked day on the Adherence tab. On Sep 18, 2026 that hid a 9-hour OT day completely: the cell showed 8:59 worked, the supervisor saw no gap, nobody coded it, and the agent went unpaid. The same week also showed a **doubled** cell on another day, where the fallback's raw non-primary login and a matching coding for the same hours both counted.

    New `wfm.utils.get_adherence_primary_resolver(agent_ids)` — a bulk-loaded predicate, `counts_for_adherence(agent_id, five9_username, on_date) -> bool`, true only for a `Five9Profile` marked `is_primary` — replaces the billable-based selection **for adherence display only**, with no fallback of any kind: an agent with no account marked primary shows zero Five9 login for display (their codings still count). This is a deliberately **separate flag and resolver from `billable`** — billing, payroll, Nómina and OT verification are untouched, still reading `wfm.utils.get_billable_username_map` exactly as before; the two must never read each other's flag or grow a fallback into one another. Every display write/read path now goes through the primary resolver: `upload_daily_file`, `rematch_daily_upload`, `_refresh_actual_hours`, `_reconcile_stale_actual_hours` (now returns the counted-agent-id set that `_zero_missing_scheduled` consumes directly, so the two can't drift apart), `_build_rows`'s weekly NR-cap sum, `finance.views._apply_live_login_hours` (Admin Adherence's live-login overwrite, now scoped to only overwrite a day that has at least one Daily Hours row — a day with none at all keeps its stored value, so a hand-entered `actual_hours` is never replaced with a fabricated 0), and `adherence.views.daily_hours_week`. The shared per-day arithmetic is now one function, `adherence.views._compute_display_hours` (pure, no queries), called by all three live paths plus the new recalculation command below — each caller keeps its own `Coding` query and admin-coding inclusion rule rather than merging them.

    On the Daily Hours page, a matched row on a non-primary account keeps Login Time visible and dashes every other column (Not Ready, Coded Time, Total Worked, NR Allowance, Excess NR, Final Hours) — primary and unmatched rows render byte-for-byte as before. `scheduling.views._save_five9_profiles` (the one Five9 write path) now auto-marks a lone Five9 account primary on save; two-or-more accounts with none chosen are left alone, deliberately, rather than guessed at.

    **Three supervisors — Jesus Urbina, Jose Aranda, Misael Martinez — intentionally have a non-billable primary account plus a billable second account.** Admin Adherence now shows near-zero login for these three by design; their real time continues to be entered as manual codings, and billing/payroll are unaffected since they keep reading the billable account. **Do not "fix" this by switching which account is primary for them or anyone else** — `is_primary` has no per-account start date yet, so `get_adherence_primary_resolver` reads today's flag for every date it's asked about, including historical weeks; switching it recomputes the past, not just the future. Per-account primary start dates are planned, not built (HANDOFF.md §8). Until then, replacing a failed Five9 account means adding the new one without deleting the old one or unchecking its Billable box, since `get_billable_username_map` has the identical lack of date-awareness and billing/payroll read today's `billable` flags for past weeks too. *(Superseded by `29203ec`: per-account primary start dates shipped, so a switch is now date-scoped and only moves days forward from its start date — the blanket warning against switching no longer applies. The advice to **add** a replacement account rather than delete it or uncheck its Billable box still stands, because `get_billable_username_map` is still date-unaware. These three supervisors' setup is deliberate and their data was not changed.)*

    Because stored `AdherenceRecord.actual_hours` is what several screens render for past weeks (the Adherence tab, My Adherence, the combined Adherence export, `adherence.views.payroll_export`, Records → Hours/Attendance, agent detail) rather than recomputing live, history written before this shipped needed a separate pass: new command `adherence/management/commands/recalculate_display_hours.py`, reusing `_compute_display_hours` — no new math. Candidates are strictly update-only (an `(agent, date)` pair qualifies only with an existing non-null `actual_hours`, a non-Official-Admin agent, and at least one `DailyAgentHours` row that date), so it can never create a row and therefore can never create one for an Official Admin. Preview by default; `--apply` runs every changed row plus one summary `log_action` entry inside a single `transaction.atomic()` — all-or-nothing, and a second `--apply` changes nothing. First production run (2026-09-25, default range 2026-08-24 to 2026-09-27): **11 agent-days updated**. **The pre-existing `recalculate_actual_hours` must never be run again** — it still selects by `billable` with a no-billable-means-count-everything fallback and hardcodes `nr_ratio=0.125`, and would put extra-account time straight back.
...
28. **Add "Shift Hours" column to combined Adherence export** (`4949f7a`) — raw regular-schedule hours Mon–Sun, excluding overtime, not zeroed by VTO/LOA/V, on both the Adherence and VTO Agents sheets. New `shift_hours` key on `_build_rows`' row dict, gated by the same `is_scheduled_day` condition as `sched_hours`.
29. **Fix stale `actual_hours` surviving Daily Hours delete/replace** (`b2a4c8b`) — added `_reconcile_stale_actual_hours(upload_date)`, wired into upload/rematch/delete, all now atomic. Display-layer only; billing/payroll unaffected since they never read `actual_hours`.
30. **Users export fields overhaul** (`ba5eba8`) — added "Agent name" as its own column, renamed "Full name"→"Legal name," dropped Django-split First/Last name columns, replaced "Phone country code" with a single formatted "Full phone number."
31. **Gate financial export columns to super admins** (`6bfe156`) — `USER_EXPORT_FINANCIAL = {'hourly_rate'}`; hidden from the picker and stripped server-side for non-super-admins.
32. **Users export: column-picker popup** (`faf1837`) — turned the fixed 8-column Users export into an 18-field (now 16 after the rename/consolidation above) checkbox picker, defaulting to the classic set, respecting current filters.
33. **Exclude LCC-employer users from three Finance exports** (`9d4c3f3`) — likely Codings/Adherence/Billing-family exports; confirmed on `codings_export` (`.exclude(employer='LCC')`).
34. **Fix admin adherence tab false "missing time" for Official Admins** (`69aae21`).
35. **Atomic agent create/edit transactions** (`30cc78d`) — prevents orphaned Django `User` rows if agent-profile creation fails mid-request.
36. **User Setup Audit export** (`5e807be`) — new super-admin, read-only, one-row-per-Five9-account audit workbook.
37. **Combined "Export Adherence" report** (`412f445`) — merges regular Adherence + Admin Adherence into one workbook via shared `_build_maps`/`_build_rows`.
38. **Admin Codings/Admin Adherence relocation, 5-part migration** (`3a5fbb9`, `494e260`, `3755b60`, `8b09140`, `a55e44e`): moved these two tabs out of the Finance section into root-level URLs (`/admin-codings/`, `/admin-adherence/`), introduced the `can_access_admin_tabs` permission flag (independent of `is_super_admin`/Finance access), added team-scoping so a non-super-admin holder sees only their own Official-Admin direct reports, and locked down server-side edit enforcement to match.
36. An **"Admin Attendance" feature was added then fully reverted** (`c1772c0` → reverted by `4f88bb9`/`f7045d2`) before the relocation work above — worth knowing if old branches/PRs reference "Admin Attendance," since it no longer exists under that name. Its `AddField` migration (`0046_agent_can_access_admin_attendance`) is the one that left the orphaned production column described in item 22.
