...
class NominaHolidayPayTests(TestCase):
    ...
    def test_not_worked_holiday_pays_scheduled_1x(self):
        # Decision 5b: scheduled but NOT worked (status 'Holiday') → 0 WORKED hours, so Holiday
        # HOURS stays 0, but the day is still paid: Holiday Pay = scheduled hours (≤8) × rate
        # (1×). Nothing is added to Hours Worked.
        from nomina.models import Holiday
        from nomina.views import _agent_nomina_data
        from adherence.models import AdherenceRecord
        from scheduling.models import Shift
    ...
    def test_not_worked_holiday_over_8_caps_at_8(self):
        # Scheduled MORE than 8h on a holiday, not worked → Holiday HOURS stays 0 (worked 0) and
        # Holiday Pay = 8 × rate (capped at 8). Regression for the uncapped case (e.g. a 9h10m
        # holiday shift at 208.33 was paying 9.166 × 208.33 = 1909.69, not 8 × 208.33 = 1666.64).
        from nomina.models import Holiday
        from nomina.views import _agent_nomina_data
        from adherence.models import AdherenceRecord
        from scheduling.models import Shift
